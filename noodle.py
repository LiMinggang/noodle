...
class Config:
        ...
        def __init__(self):
                ...
                s = ''.join([random.choice(string.ascii_letters + string.digits) for n in range(8192)])
                self.gBuff = s.encode()
        ...
        def parse_args(self, args=None):
                ...
                if not self.args.l:
                    self.args.l = min(int(self.args.b/16), 1470)
                print("per conn bw:", self.args.b, "real BW:", self.args.b*self.args.C, "conns per thread ramp=",self.args.n, "msg-size=", self.args.l)
                return

class Connection():
        def __init__(self, socket_mode, msg_size, gBuff, bit_rate, daddr, dport, saddr=0, sport=0):
                self.saddr = saddr
                self.sport = sport
                self.daddr = daddr
                self.dport = dport
                ...
                self.buf = gBuff[2:self.msg_size]
        ...
        def send(self):
                if self.sent_this_second < self.send_per_second:
                                try: 
                                        self.sent_this_second += self.socket.sendto(self.buf,(self.daddr, self.dport))
...
# Manage Total/threads sessions within a thread.
        def __init__(self, idx, config):
                ...
                for i in range(self.num_conns):
                        self.conns.append(Connection(config.socket_mode, self.config.args.l, config.gBuff, self.config.args.b, self.config.args.c, self.config.args.p))
